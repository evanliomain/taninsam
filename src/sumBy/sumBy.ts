 ...
 * @param iteratee The iteratee invoked per element to get the value to sum given an element of the array
 ...
 * ```
 * sumBy<{ a: number }>(x => x.a)([{ a: 1 }, { a: 2 }, { a: 3 }, { a: 4 }]) // 10
 * ```
...
export function sumBy<T>(
  ...
  return (array: T[], links?: Links) =>
    array.reduce(
      (sum, data, index: number, array: ReadonlyArray<T>) =>
        sum + iteratee(data, index, array, links),
      0
    );
}
